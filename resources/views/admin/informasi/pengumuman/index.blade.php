...
<div class="flex flex-col justify-center items-center p-5 mb-5">
  <h1 class="text-[40px] text-gray-400 monserrat font-bold">PENGUMUMAN</h1>
</div>
<div class="fixed bottom-0 end-0 m-10 w-[50px] h-[50px] rounded-lg bg-sky-600">
  <a href="{{ route('admin.announcement.create') }}" class="font-bold text-white text-3xl w-full h-full flex justify-center items-center ">
    +
  </a>
</div>
<div class="px-16">
  <div class="flex gap-3 items-center justify-end mb-5">
    <a href="{{ route('admin.announcement.create') }}" class="px-3 py-2 rounded-lg bg-sky-600 text-white">Buat Data</a>
  </div>
  ...
  <table class="w-full">
    ...
    <tbody>
      ...
      <tr class="border text-[14px] even:bg-gray-100 odd:bg-white">
        <th class="text-start p-3 border">{{ $loop->iteration }}</th>
        <td class="text-start p-3 border">
          <a href="{{ route('admin.announcement.show', $item->uuid) }}" class="text-sky-600 hover:underline">{{ $item->title }}</a>
        </td>
        ...
        <td class="text-start p-3 flex items-center justify-center gap-2">
          <a href="{{ route('admin.announcement.edit', $item->uuid) }}" class="px-3 py-2 rounded-lg bg-orange-400 text-white"><i class="bi bi-pencil-square"></i></a>
          <form action="{{ route('admin.announcement.delete', $item->uuid) }}" method="post">
            @method('delete')
            @csrf
