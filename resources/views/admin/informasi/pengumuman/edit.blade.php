...
<div class="p-16 flex justify-center">
  <div class="w-[80%]">
    <form action="{{ route('admin.announcement.update', $announcement->uuid) }}" method="post" class="w-full" enctype="multipart/form-data">
      @csrf
      @method('put')
      <div class="mb-5">
        <label for="title" class="font-semibold mb-3">Judul Pengumuman</label>
        <input type="text" name="title" id="title" class="w-full @error('title') border border-red-500 @enderror p-3 rounded border border-gray-600 outline-gray-700" value="{{ old('title', $announcement->title) }}">
        @error('title')
      ...
      <div class="mb-5">
        ...
        <input type="date" name="date" id="date" class="w-full @error('date') border border-red-500 @enderror p-3 rounded border border-gray-600 outline-gray-700" value="{{ old('date', $announcement->date) }}">
        @error('date')
        <div class="text-xs text-red-500">
          {{ $message }}
        </div>
      ...
      <div class="mb-3 border-b">
        ...
        <embed id="pdf-preview" class="w-full h-[260px] border rounded-lg mb-3" src="{{ asset('storage/' . $announcement->file) }}" type="application/pdf">
        <input type="hidden" name="oldFile" value="{{ $announcement->file }}">
        @else
        <embed id="pdf-preview" class="w-full h-[260px] border rounded-lg mb-3" type="application/pdf" style="display:none;">
        @endif
