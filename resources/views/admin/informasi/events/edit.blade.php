...
<div class="p-16 flex justify-center">
  <div class="w-[80%]">
    <form action="{{ route('admin.event.update', $event->uuid) }}" method="post" class="w-full" enctype="multipart/form-data">
      @csrf
      @method('put')
      <div class="mb-3 border-b">
        <p class="my-3 font-semibold">Gambar Terkait</p>
        @if ($event->image1)
        <img class="img-preview w-full h-[260px] border rounded-lg mb-3 object-cover" src="{{ asset('storage/' . $event->image1) }}">
        <input type="hidden" name="oldImage" value="{{ $event->image1 }}">
        @else
      ...
      <div class="mb-5">
        ...
        <input type="text" name="title" id="title" class="w-full @error('title') border border-red-500 @enderror p-3 rounded border border-gray-600 outline-gray-700" value="{{ old('title', $event->title) }}">
        @error('title')
      ...
      <div class="mb-5">
        ...
        <input type="date" name="start_event" id="start_event" class="w-full @error('start_event') border border-red-500 @enderror p-3 rounded border border-gray-600 outline-gray-700" value="{{ old('start_event', $event->start_event) }}">
        @error('start_event')
        <div class="text-xs text-red-500">
          {{ $message }}
        </div>
      ...
      <div class="mb-5">
        ...
        <input type="date" name="end_event" id="end_event" class="w-full @error('end_event') border border-red-500 @enderror p-3 rounded border border-gray-600 outline-gray-700" value="{{ old('end_event', $event->end_event) }}">
        @error('end_event')
        <div class="text-xs text-red-500">
          {{ $message }}
        </div>
